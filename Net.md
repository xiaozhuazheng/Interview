...
### 8、TCP如何控制流量
流量控制主要是避免发送端向接收端发送的数据过多，超过了接收端处理能力，从而进行流量控制，让数据传输端了解数据接收端当前的接收能力，可灵活调整传输速率。在TCP中，通过“滑动窗口”来实现：</br>
![avatar](/image/net_contrl.png)  </br>
1、发送端滑动窗口为500个字节，向接收端发送400个字节的数据；</br>
2、发送端收到了前200个字节的确认，滑动窗口不变，还可以发送300个字节；</br>
3、发送端收到已发送的400个字节的收到确认消息，但接收端通知需要把窗口缩小到400个字节，因此发送端的滑动窗口向前滑动100个字节，最多发送400个字节。
